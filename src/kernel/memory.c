...
int memory_free(void *address, unsigned int size)
{
	...
	for (i = 0; i < memoryManegement.memorySegmentLength; ++i) {
		if (memoryManegement.memorySegment[i].address > (unsigned int) address)
			break;
	}

	if (i > 0) {
		if (memoryManegement.memorySegment[i - 1].address + memoryManegement.memorySegment[i - 1].size == (unsigned int) address) {
			memoryManegement.memorySegment[i - 1].size += size;
			memoryManegement.freeMemorySize += size;
			if (i < memoryManegement.memorySegmentLength) {
				if ((unsigned int) address + size  == memoryManegement.memorySegment[i].address) {
	...
	if (i < memoryManegement.memorySegmentLength) {
		if ((unsigned int) address + size == memoryManegement.memorySegment[i].address) {
			memoryManegement.memorySegment[i].address = (unsigned int) address;
			memoryManegement.memorySegment[i].size 	+= size;
			memoryManegement.freeMemorySize += size;
			return 0;
		}
...
void *memory_allocate_4k(unsigned int size)
{
	...
	address = (unsigned int)memory_allocate(size);
	return (void *)address;
}
