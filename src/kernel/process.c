...
int *KERNEL_CODE_START 			= &_text;
int *KERNEL_CODE_END 			= &_etext;
int *KERNEL_DATA_START 			= &_data;
int *KERNEL_DATA_END 			= &_edata;
...
void init_task()
{
	...
	pTaskIdle->tss32.ebp = (unsigned int) memory_allocate_4k(STACK_SIZE) + STACK_SIZE;
	pTaskIdle->tss32.esp = pTaskIdle->tss32.ebp;
...
void task_idle()
{
	while (1) {
		cli();
		if (!isEmpty(&keyboardMessageQueue) || !isEmpty(&kernelMessageQueue)) {
			pTaskIdle->flags = TASK_READY;
    		pTaskKernel->flags = TASK_RUNNING;
    		taskManegement.currentTask = pTaskKernel;
